			...
			if (g_hat(v) + cvx + h_hat(x) < g_T(goal_node)) {
				...
				if (g_T(v) + cvx < g_T(x)) {
					...
					if (curr_time - last_time > report_interval) {
						...
						ofstream outfile("BITSTAR_double_integrator_exact_statistics_" + std::to_string(setup_values.max_time) + "_seconds_run_" + std::to_string(setup_values.stats_id) + ".txt", ios::app);
						if (outfile.is_open()) {
