#ifndef DOUBLE_INTEGRATOR_EXACT_CONNECTOR_HPP_
#define DOUBLE_INTEGRATOR_EXACT_CONNECTOR_HPP_
...
//
#define DELTA_T 1             // The time interval of integration and node placement
...
typedef Matrix<double, 3, 1> VectorU_extend; // format: [delta, u1, u2]
typedef std::vector<VectorU_extend> StdVectorU_extend;
typedef Matrix<double, 4, 1> VectorX;
...
int extend_with_optimal_control (VectorXd state_ini, VectorXd state_fin, 
			       StdVectorX& traj_states, StdVectorU_extend& traj_inputs) {
  ...
  while (times_counter < 6) { 
    ...
    traj_states.push_back(state_new);
    traj_inputs.push_back(input_new);
...
int extend (VectorXd state_from_in, VectorXd state_towards_in,
	  int *exact_connection_out, StdVectorX& traj_states, StdVectorU_extend& traj_inputs) {
