...
/* copy functions */
void copyCArrayToM(double_integrator_QP_solver_noCD_exp_FLOAT *src, double *dest, int dim) {
    while (dim--) {
        *dest++ = (double)*src++;
    }
}
void copyMArrayToC(double *src, double_integrator_QP_solver_noCD_exp_FLOAT *dest, int dim) {
    while (dim--) {
        *dest++ = (double_integrator_QP_solver_noCD_exp_FLOAT)*src++;
    }
}
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )  
{
	...
	const char *infofields[15] = { "it",
							   ...
							   "step_cc",
							   "solvetime"};
	double_integrator_QP_solver_noCD_exp_params params;
	double_integrator_QP_solver_noCD_exp_output output;
	double_integrator_QP_solver_noCD_exp_info info;
	
	/* Check for proper number of arguments */
    if (nrhs != 1) {
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help double_integrator_QP_solver_noCD_exp_mex' for details.");
    }    
	if (nlhs > 3) {
    ...
    mexErrMsgTxt("PARAMS.lb1 must be of size [5 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.lb1, 5);
    ...
    mexErrMsgTxt("PARAMS.ub1 must be of size [4 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.ub1, 4);
    ...
    mexErrMsgTxt("PARAMS.A11 must be of size [22 x 15]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.A11, 330);
	...
	/* call solver */
	exitflag = double_integrator_QP_solver_noCD_exp_solve(&params, &output, &info);
