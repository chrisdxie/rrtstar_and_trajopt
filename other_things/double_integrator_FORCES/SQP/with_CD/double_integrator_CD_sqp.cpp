...
double_integrator_QP_solver_CD_FLOAT **f, **lb, **ub, **A, **b, **z;
}
...
// Simplicity is key
#define Z_DIM_1_TO_N_MINUS_2 2*X_DIM+2*U_DIM+1+O_DIM+X_DIM
#define Z_DIM_N_MINUS_1 2*X_DIM+U_DIM+1+O_DIM+X_DIM
...
void setup_state_vars(double_integrator_QP_solver_CD_params& problem, double_integrator_QP_solver_CD_output& output)
{
...
void fill_lb_and_ub(bounds_t bounds) {
	...
	for(int i = 0; i < LB_DIM_1; ++i) { // Taking advantage of fact that LB_DIM_1 = UB_DIM_1 + 1
		if (i < X_DIM) { lb1_temp(i) = bounds.x_start(i) ; ub1_temp(i) = bounds.x_start(i); } // Enforcing start state
...
//		std::cout << "ub" << t+1 << ":\n" << ubt_temp << "\n";
	...
	for(int i = 0; i < LB_DIM_N; ++i) {
		if (i < X_DIM) { lbN_temp(i) = bounds.x_goal(i); ubN_temp(i) = bounds.x_goal(i); } // Enforcing goal state
	}
...
void fill_in_A_and_b(StdVectorX& X, StdVectorU& U, double* delta, double trust_box_size, bounds_t bounds) {
	...
	for(int t = 0; t < T-1; ++t) {
		...
		Matrix<double, A_DIM_OUTPUT_1_TO_N_MINUS_1, Z_DIM_1_TO_N_MINUS_2> A_temp;
		...
		// Swept out Volume
		A_temp.middleRows(curr_row, O_DIM) << MatrixXd::Zero(O_DIM, 2*X_DIM+2*U_DIM+1), -1*MatrixXd::Identity(O_DIM, O_DIM), MatrixXd::Zero(O_DIM, X_DIM);
		curr_row += O_DIM;
		...
		A_temp.middleRows(curr_row, O_DIM) << jacx, MatrixXd::Zero(O_DIM, U_DIM), jacxnext, MatrixXd::Zero(O_DIM, U_DIM+1), -1*MatrixXd::Identity(O_DIM, O_DIM), MatrixXd::Zero(O_DIM, X_DIM);
		b_temp.middleRows(curr_row, O_DIM) << jacx*x + jacxnext*x_next - swval;
		curr_row += O_DIM;
		...
		A_temp.middleRows(curr_row, X_DIM) << DH_X, DH_U, MatrixXd::Identity(X_DIM, X_DIM), MatrixXd::Zero(X_DIM, U_DIM), DH_delta, MatrixXd::Zero(X_DIM, O_DIM), -1*MatrixXd::Identity(X_DIM, X_DIM);
		b_temp.middleRows(curr_row, X_DIM) << DH*x_prime - hval;
		curr_row += X_DIM;
		A_temp.middleRows(curr_row, X_DIM) << -1*DH_X, -1*DH_U, -1*MatrixXd::Identity(X_DIM, X_DIM), MatrixXd::Zero(X_DIM, U_DIM), -1*DH_delta, MatrixXd::Zero(X_DIM, O_DIM), -1*MatrixXd::Identity(X_DIM, X_DIM);
		b_temp.middleRows(curr_row, X_DIM) << hval - DH*x_prime;
		curr_row += X_DIM;

		// Trust box stuff
		A_temp.middleRows(curr_row, X_DIM+U_DIM+1) << MatrixXd::Identity(X_DIM+U_DIM+1, X_DIM+U_DIM), MatrixXd::Zero(X_DIM+U_DIM+1, X_DIM+U_DIM+1+O_DIM+X_DIM);
		...
		A_temp.middleRows(curr_row, X_DIM+U_DIM+1) << -1*MatrixXd::Identity(X_DIM+U_DIM+1, X_DIM+U_DIM), MatrixXd::Zero(X_DIM+U_DIM+1, X_DIM+U_DIM+1+O_DIM+X_DIM);
		A_temp(curr_row+X_DIM+U_DIM, 2*X_DIM+2*U_DIM) = -1;
		...
		if (t == T-2) {
			...
			A_N_MINUS_1_temp << A_temp.middleCols(0, 2*X_DIM+U_DIM), A_temp.rightCols(1+O_DIM+X_DIM);
			fill_col_major(A[t], A_N_MINUS_1_temp);
...
double computeMerit(double* delta, StdVectorX& X, StdVectorU& U, double penalty_coeff, bounds_t bounds) {
	...
	// Swept out volume
	for(int t = 0; t < T-1; ++t) {
		// Get matrix of values and jacobians, only want values though.
		MatrixXd swv = swept_out_volume_collision_and_jacobian(X[t], X[t+1], bounds.d_safe, bounds.obstacles);
		for(int i = 0; i < swv.rows(); ++i) {
			if (swv(i, 0) > 0) { // Add value if it's positive, this gives sum of |g(x, x_next)|^+
				merit += penalty_coeff*swv(i, 0);
			}
		}
	}
...
bool minimize_merit_function(StdVectorX& X, StdVectorU& U, double* delta, bounds_t bounds, double penalty_coeff,
		double_integrator_QP_solver_CD_params& problem, double_integrator_QP_solver_CD_output& output, double_integrator_QP_solver_CD_info& info) {
...
//			std::cout << "   trust region size: " << trust_box_size << "\n";
			...
			// fill in A, b
			fill_in_A_and_b(X, U, delta, trust_box_size, bounds);
			...
			// call FORCES
			int exitflag = double_integrator_QP_solver_CD_solve(&problem, &output, &info);
			if (exitflag == 1) {
				optcost = info.pobj;
 ...
 */
int solve_double_integrator_CD_BVP(VectorX x_start, VectorX x_goal, StdVectorX& X, StdVectorU& U, double* delta, bounds_t bounds) {

	double_integrator_QP_solver_CD_params problem;
	double_integrator_QP_solver_CD_output output;
	double_integrator_QP_solver_CD_info info;
	setup_state_vars(problem, output);

	// Assume pointer is NOT null
	*delta = 1;
...
int main() {
	...
	bounds.d_safe = 0.05;
	bounds.obstacles = obstacles;

	int success = solve_double_integrator_CD_BVP(x_start, x_goal, X, U, delta, bounds);

	if (success == 1) {
		printf("SUCCESS!!!\n");
		printf("Optimal time: %f\n", *delta);
	}
