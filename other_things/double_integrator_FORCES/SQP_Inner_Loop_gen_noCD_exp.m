function SQP_Inner_Loop_gen_noCD_exp()
...
% This file is to generate FORCES code to test our SQP method against exact
% solutions to linear dynamic systems. We want to compare how our method
% fares against the EXACT solutions, which is shown how to compute in this
% paper: http://arl.cs.utah.edu/pubs/ICRA2013-1.pdf
...
stages(i).cost.H(nX+1,nX+1) = 2;                            % Quadratic cost for controls
stages(i).cost.H(nX+2,nX+2) = 2;                            % Quadratic cost for controls
params(1) = newParam(['f' i_str], i, 'cost.f');             % Parameter for cost

% Lower Bounds
stages(i).ineq.b.lbidx = [1:nX 2*nX+2*nU+1];             % Lower bounds on states, and time
params(end+1) = newParam(['lb' i_str], i, 'ineq.b.lb');

% Upper bounds
stages(i).ineq.b.ubidx = 1:stages(i).dims.u;                % Upper bounds on states, NOT time
...
for i=2:N-2
    ...
    % Dimensions
    stages(i).dims.n = 2*nX+2*nU+1+nX;                          % number of stage variables
    stages(i).dims.l = 0;                                    % number of lower bounds
    stages(i).dims.u = 0;                                    % number of upper bounds
    stages(i).dims.r = nX+nU+1;                                 % number of equality constraints
    stages(i).dims.p = 4*nX+2*nU+2;                             % number of affine constraints
    stages(i).dims.q = 0;                                       % number of quadratic constraints
...
solver_name = 'double_integrator_QP_solver_noCD_exp';
