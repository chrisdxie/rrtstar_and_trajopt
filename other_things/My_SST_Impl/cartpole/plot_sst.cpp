...
py::object init_display() {
	...
	std::string working_dir = "/Users/ChrisXie/school/research/RRTSTAR_TrajOpt_Project/RRTSTAR_and_TrajOpt/other_things/My_SST_Impl/cartpole";
	...
	py::object plotter = plot_mod.attr("cartplot"); // CHANGE BACK TO PLOT
	return plotter;
}
...
void plot(py::object plotter, np::ndarray states, double w, double h, double l) {

    bool pause = true;

	try {
	    // pass control to python now
		plotter(states, w, h, l);
	}
	catch(py::error_already_set const &) {
	    // will pass python errors to cpp for printing
		PyErr_Print();
...
void cartpole_MonteCarlo_prop(Node* prop_node, Node* new_node) {

	VectorXd x_prop = prop_node->state;

	double prop_time = uniform(0, setup_values.T_prop);
	double u = uniform(setup_values.u_min, setup_values.u_max);

	VectorXd controls(1);
	controls << u;
	VectorXd x_new = rk4(continuous_cartpole_dynamics, x_prop, controls, prop_time);
...
double SST() {
	...
	while (k <= setup_values.max_iters) {
		...
		Node* x_new = new Node;
		cartpole_MonteCarlo_prop(x_nearest, x_new);
...
int main(int argc, char* argv[]) {
   ...
   //  std::cout << "exiting\n";
    ...
    // Cartpole set parameters
	py::object plotter = init_display(); // Initialize python interpreter and pyplot plot

	double cart_width = .5;
	double cart_height = .2;
	double cart_mass = 1;
	double pole_mass = .3;
	double pole_length = 2;
    dynamics_library::set_cartpole_parameters(cart_mass, pole_mass, pole_length);

    Node* root = new Node;
    Node* curr = root;

    curr->state = setup_values.initial_state;
    for (int i = 0; i < 10; i++) {
    	Node* next = new Node;
    	cartpole_MonteCarlo_prop(curr, next);
    	curr->children.insert(next);
    	next->parent = curr;
    	curr = next;
    }
	np::ndarray states_np = eigen_to_ndarray(tree_to_matrix_states(root));

	plot(plotter, states_np, cart_width, cart_height, pole_length);
}
