...
MatrixXd get_path(Node* x) {
	MatrixXd P(2, setup_values.max_iters);
	int index = 0;
...
void point_robot_MonteCarlo_prop(Node* prop_node, Node* new_node) {

	VectorXd x_prop = prop_node->state;
	//double prop_time = uniform(0, setup_values.T_prop);
	double u1 = uniform(-1, 1);
	double u2 = uniform(-1, 1);

	Vector2d prop_vec;
	prop_vec << u1, u2;
	VectorXd x_new = x_prop + prop_vec;

	new_node->state = x_new;
	new_node->parent = prop_node;
	new_node->cost = prop_node->cost + prop_vec.norm();
...
void SST() {
	...
	while (k <= setup_values.max_iters) {
		...
		Vector2d sample = sample_state();
		Node* x_nearest = bestNear(V_active, sample, setup_values.delta_v);

		// Monte Carlo propagation
		Node* x_new = new Node;
		point_robot_MonteCarlo_prop(x_nearest, x_new);
