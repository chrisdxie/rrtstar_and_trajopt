...
% Upper bounds
stages(i).ineq.b.ubidx = [1:nX+nU 2*nX+2*nU+1];              % Upper bounds on states, controls, NOT time
...
for i=2:N-2
    ...
    % Lower Bounds
    stages(i).ineq.b.lbidx = 5;                % Lower bounds on control
    params(end+1) = newParam(['lb' i_str], i, 'ineq.b.lb');
    
    % Upper bounds
    stages(i).ineq.b.ubidx = 5;                % Upper bounds on controls
    params(end+1) = newParam(['ub' i_str], i, 'ineq.b.ub');
...
% Upper bounds
stages(i).ineq.b.ubidx = 5;                % Upper bounds on control
params(end+1) = newParam(['ub' i_str], i, 'ineq.b.ub');
...
% Lower bounds
stages(i).ineq.b.lbidx = 1:stages(i).dims.l;                % Lower bounds on state
params(end+1) = newParam(['lb' i_str], i, 'ineq.b.lb');

% Upper bounds
stages(i).ineq.b.ubidx = 1:stages(i).dims.u;                % Upper bounds on state
params(end+1) = newParam(['ub' i_str], i, 'ineq.b.ub');
...
solver_name = 'cartpole_QP_solver';
