...
cartpole_QP_solver_FLOAT **f, **lb, **ub, **A, **b, **z;
}
...
#define LB_DIM_2_TO_N_MINUS_1 U_DIM
#define UB_DIM_2_TO_N_MINUS_1 U_DIM
...
const double initial_penalty_coeff = 0.001;
const double initial_trust_box_size = .0001;
...
void setup_state_vars(cartpole_QP_solver_params& problem, cartpole_QP_solver_output& output)
{
	...
	// problem inputs
	f = new cartpole_QP_solver_FLOAT*[T];
	lb = new cartpole_QP_solver_FLOAT*[T];
	ub = new cartpole_QP_solver_FLOAT*[T];
	A = new cartpole_QP_solver_FLOAT*[T];
	b = new cartpole_QP_solver_FLOAT*[T];
...
void fill_lb_and_ub(bounds_t bounds) {
	...
	for(int t = 1; t < T-1; ++t) {
		...
		for (int i = 0; i < LB_DIM_2_TO_N_MINUS_1; ++i) {
			lbt_temp(i) = bounds.u_min; ubt_temp(i) = bounds.u_max; // Control bounds
		}
...
void fill_in_A_and_b(StdVectorX& X, StdVectorU& U, double* delta, double trust_box_size) {
	...
	for(int t = 0; t < T-1; ++t) {
		...
		Matrix<double, X_DIM, X_DIM+U_DIM+1> jac = -1 * cartpole::numerical_jacobian(cartpole::continuous_dynamics, x, u, d);
		...
		VectorXd hval = cartpole::dynamics_difference(cartpole::continuous_dynamics, x, x_next, u, d);
...
double computeMerit(double* delta, StdVectorX& X, StdVectorU& U, double penalty_coeff) {
	...
	for(int t = 0; t < T-1; ++t) {
		VectorXd hval = cartpole::dynamics_difference(cartpole::continuous_dynamics, X[t], X[t+1], U[t], *delta);
 ...
 */
int solve_double_integrator_BVP(VectorX x_start, VectorX x_goal, StdVectorX& X, StdVectorU& U, double* delta, bounds_t bounds) {
