...
rally_car_QP_solver_FLOAT **f, **lb, **ub, **C, **e, **z;
}
...
const int max_penalty_coeff_increases = 4;
...
void setup_state_vars(rally_car_QP_solver_params& problem, rally_car_QP_solver_output& output)
{

	/* Initialize problem inputs and outputs as double arrays */
	// problem inputs
	f = new rally_car_QP_solver_FLOAT*[T-1];
	lb = new rally_car_QP_solver_FLOAT*[T];
	ub = new rally_car_QP_solver_FLOAT*[T];
	C = new rally_car_QP_solver_FLOAT*[T-1];
	e = new rally_car_QP_solver_FLOAT*[T-1];

	// problem outputs
	z = new rally_car_QP_solver_FLOAT*[T];
...
void fill_lb_and_ub(StdVectorX& X, StdVectorU& U, double& delta, double trust_box_size, bounds_t bounds)
{
	...
	for(int i = 0; i < X_DIM; ++i) {
		...
		if ( (2 <= i && i <= 3) || (5 <= i && i <= 7)) { // Free velocities
...
void fill_in_C_and_e(StdVectorX& X, StdVectorU& U, double& delta, double trust_box_size, bounds_t bounds)
{
	...
	VectorX xt1;
	Matrix<double, X_DIM, X_DIM+U_DIM+1> jac = numerical_jacobian(continuous_rally_car_dynamics, x0, u0, delta);
	...
	xt1 = rk4(continuous_rally_car_dynamics, x0, u0, delta);
	...
	for(int t = 1; t < T-1; ++t)
	{
		...
		xt1 = rk4(continuous_rally_car_dynamics, xt, ut, delta);
		jac = numerical_jacobian(continuous_rally_car_dynamics, xt, ut, delta);
...
double computeMerit(double& delta, StdVectorX& X, StdVectorU& U, double penalty_coeff) {
	...
	for(int t = 0; t < T-1; ++t) {
		VectorXd hval = dynamics_difference(continuous_rally_car_dynamics, X[t], X[t+1], U[t], delta);
...
bool minimize_merit_function(StdVectorX& X, StdVectorU& U, double& delta, bounds_t bounds, double penalty_coeff,
		rally_car_QP_solver_params& problem, rally_car_QP_solver_output& output, rally_car_QP_solver_info& info) {
	...
	for(sqp_iter=0; sqp_iter < cfg::max_sqp_iterations; ++sqp_iter) {
		...
		for(int trust_iter=0; true; ++trust_iter) {
			...
			// call FORCES
			int exitflag = rally_car_QP_solver_solve(&problem, &output, &info);
			if (exitflag == 1) {
				optcost = info.pobj;
...
bool penalty_sqp(StdVectorX& X, StdVectorU& U, double& delta, bounds_t bounds,
	...
	while(penalty_increases < cfg::max_penalty_coeff_increases) {
		...
		// warm start?
		for(int t = 0; t < T-2; ++t) {
			X[t+1] = rk4(continuous_rally_car_dynamics, X[t], U[t], delta);
		}
	}
...
int solve_rally_car_BVP(StdVectorX& X, StdVectorU& U, double& delta, bounds_t bounds) {

	rally_car_QP_solver_params problem;
...
int main(int argc, char* argv[]) {
	...
	x_start << atof(argv[2]), atof(argv[3]), atof(argv[4]), atof(argv[5]), atof(argv[6]), atof(argv[7]), atof(argv[8]), atof(argv[9]);
	x_goal << atof(argv[10]), atof(argv[11]), atof(argv[12]), atof(argv[13]), atof(argv[14]), atof(argv[15]), atof(argv[16]), atof(argv[17]);

	// Init bounds
	bounds_t bounds;

	// Taken from SST rally car example
	bounds.u_min << -1.0472, -700, -700;
	bounds.u_max <<  1.0472,    0, 1200;
	bounds.x_min << -25, -40, -18, -18, -3*M_PI, -17, -40, -40;
	bounds.x_max <<  25,  25,  18,  18,  3*M_PI,  17,  40,  40;
	bounds.delta_min = 0;
	bounds.delta_max = 2;
	...
	bool success = solve_rally_car_BVP(X, U, delta, bounds);
