...
double_integrator_QP_solver_FLOAT **f, **lb, **ub, **C, **e, **z;
}
...
void setup_state_vars(double_integrator_QP_solver_params& problem, double_integrator_QP_solver_output& output)
{
	...
	// problem inputs
	f = new double_integrator_QP_solver_FLOAT*[T-1];
	lb = new double_integrator_QP_solver_FLOAT*[T];
	ub = new double_integrator_QP_solver_FLOAT*[T];
	C = new double_integrator_QP_solver_FLOAT*[T-1];
	e = new double_integrator_QP_solver_FLOAT*[T-1];
...
void fill_in_C_and_e(StdVectorX& X, StdVectorU& U, double& delta, double trust_box_size, bounds_t bounds)
{
	...
	VectorX xt1;
	Matrix<double, X_DIM, X_DIM+U_DIM+1> jac = numerical_jacobian(continuous_double_integrator_dynamics, x0, u0, delta);
	...
	xt1 = rk4(continuous_double_integrator_dynamics, x0, u0, delta);
	...
	for(int t = 1; t < T-1; ++t)
	{
		...
		xt1 = rk4(continuous_double_integrator_dynamics, xt, ut, delta);
		jac = numerical_jacobian(continuous_double_integrator_dynamics, xt, ut, delta);
 ...
 */
int solve_double_integrator_BVP(StdVectorX& X, StdVectorU& U, double& delta, bounds_t bounds) {
	...
	delta = (bounds.x_start - bounds.x_goal).norm()/10.0;
...
int main() {
	...
	bounds.u_min << -1, -1;
	bounds.u_max << 1, 1;
	bounds.x_min << -10, -10, -1, -1;
	bounds.x_max << 10, 10, 1, 1;
	bounds.delta_min = 0;
	bounds.delta_max = 10;
	...
	bool success = solve_double_integrator_BVP(X, U, delta, bounds);
