...
::mc_update_insert_vertex (vertex_t *vertex_in) {
  ...
  for (int i = 0; i < NUM_DIMENSIONS; i++)  {
    if (fabs (vertex_in->state->state_vars[i] - region_goal.center[i]) > region_goal.size[i]/2.0) {
...
::get_solution (trajectory_t &trajectory_out) {
  ...
  vertex_t *vertex_curr = min_cost_vertex;
  
  
  // The solution will eventually reach a root node
  while (1) {
    
    // Add the current state to the trajectory
    state_t *state_curr = vertex_curr->state;
    trajectory_out.push_front (state_curr);

    if ( vertex_curr->incoming_edges.empty() ) {
      break;
    }
    
    
    // Add the trajectory that connects the current state to its parent
    //    to the trajectory.
    edge_t *edge_curr = vertex_curr->incoming_edges.front();
    trajectory_t *trajectory_curr = edge_curr->trajectory_edge;
    trajectory_out.push_front (trajectory_curr);

    vertex_curr = edge_curr->vertex_src;
  }
